...
return {
	...
	time = function(v, p, t, e, font_type)
		...
		drawf(v, font_type, 313*FRACUNIT, (hudinfo[HUD_SECONDS].y-21)*FRACUNIT, FRACUNIT, "T"..mint.."'"..sect..'"'..cent, (hudinfo[HUD_RINGS].f|V_SNAPTORIGHT) &~ V_SNAPTOLEFT, v.getColormap(TC_DEFAULT, 0), "right", -1, 0)
	end,

	rings = function(v, p, t, e, font_type)
		v.draw(2, 1, v.cachePatch("RINGCOLR"), hudinfo[HUD_RINGS].f, v.getColormap(TC_DEFAULT, p.mo.color))
		drawf(v, font_type, (hudinfo[HUD_RINGSNUM].x-59)*FRACUNIT, (hudinfo[HUD_SECONDS].y-20)*FRACUNIT, FRACUNIT, p.rings, hudinfo[HUD_RINGS].f, v.getColormap(TC_DEFAULT, 0), "right", -1, 3, '0')
		if p.rings < 1 and (leveltime % 8) / 4 then
			drawf(v, font_type, (hudinfo[HUD_RINGSNUM].x-59)*FRACUNIT, (hudinfo[HUD_SECONDS].y-20)*FRACUNIT, FRACUNIT, "RRR", hudinfo[HUD_RINGS].f, v.getColormap(TC_DEFAULT, 0), "right", -1, 0)
		end
	end,
	...
	lives = function(v, p, t, e, font_type, icon_style, bot_existance, bot_skin, bot_color)
		...
		if icon_style and bot_existance then
			if bot_existance.valid then
				...
				drawf(v, 'COLCNT', (hudinfo[HUD_LIVES].x+7)*FRACUNIT, (hudinfo[HUD_LIVES].y+10)*FRACUNIT, FRACUNIT, "X"..lives, hudinfo[HUD_LIVES].f, v.getColormap(TC_DEFAULT, 0), 0, -1, 0)
			else
				draw_lifeicon(v, hudinfo[HUD_LIVES].x-2, hudinfo[HUD_LIVES].y+13, v.getSprite2Patch(p.mo.skin, SPR2_LIFE, false, A, 0), hudinfo[HUD_LIVES].f|(icon_style == nil and 0 or V_FLIP), {1, p.mo.color})
				drawf(v, 'COLCNT', (hudinfo[HUD_LIVES].x+3)*FRACUNIT, (hudinfo[HUD_LIVES].y+10)*FRACUNIT, FRACUNIT, "X"..lives, hudinfo[HUD_LIVES].f, v.getColormap(TC_DEFAULT, 0), 0, -1, 0)
				bot_existance = nil
			end
		else
			draw_lifeicon(v, hudinfo[HUD_LIVES].x-2, hudinfo[HUD_LIVES].y+13, v.getSprite2Patch(p.mo.skin, SPR2_LIFE, false, A, 0), hudinfo[HUD_LIVES].f|(icon_style == nil and 0 or V_FLIP), {1, p.mo.color})

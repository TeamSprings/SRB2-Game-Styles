...
--TBS_FontDrawer(v, font, x, y, scale, value, flags, color, alligment, padding, leftadd, symbol)
...
local function checkpointTimeDisplay(v, p)
	if p.checkpointtime then
		if (leveltime % 4)/2 then
			...
			TBS_FontDrawer(v, 'SA2TL', (290)*FRACUNIT, (240)*FRACUNIT, FRACUNIT-FRACUNIT/4, mint..':'..sect..':'..cent,
...
local function tallyDrawer(v, p)
	...
	if transparency[index] ~= V_90TRANS then
		...
		TBS_FontDrawer(v, 'SA2TL', FixedDiv((392-scorelen*6)*textscaling[index]-scorelen*textscaling[index]/4,
	...
	if transparency[index] ~= V_90TRANS then
		...
		TBS_FontDrawer(v, 'SA2TL', FixedDiv(350*textscaling[index], textscaling[index]), ztime-textscaling[index],
	...
	if transparency[index] ~= V_90TRANS then
		...
		TBS_FontDrawer(v, 'SA2TL', FixedDiv((392-ringslen*6)*textscaling[index]-ringslen*textscaling[index]/4,
	...
	if transparency[index] ~= V_90TRANS then
		...
		TBS_FontDrawer(v, 'SA2TL', FixedDiv((392-timelen*6)*textscaling[index]-timelen*textscaling[index]/4,
	...
	if transparency[index] ~= V_90TRANS then
		...
		TBS_FontDrawer(v, 'SA2TL', FixedDiv((392-totallen*6)*textscaling[index]-totallen*textscaling[index]/4,
...
hud.add(function(v, p, t, et)
	...
	if t < et then
		...
		TBS_FontDrawer(v, 'SA2TTFONT', (116+easespin-easespout)*FRACUNIT, 82*FRACUNIT, FRACUNIT-FRACUNIT/4, "Stage: "..stagenum, hudinfo[HUD_LIVES].f|V_PERPLAYER, v.getColormap(TC_DEFAULT, SKINCOLOR_SLATE), 0, -1, 0)
		...
		if subtitle and subtitle ~= "" then
			...
			if t > TICRATE then
				TBS_FontDrawer(v, 'COMSANSFT', 300*FRACUNIT, 300*FRACUNIT, FRACUNIT/2, subtitle, hudinfo[HUD_LIVES].f|(easetransparency3 << V_ALPHASHIFT)|V_PERPLAYER, v.getColormap(TC_DEFAULT, SKINCOLOR_CARBON), "center", 1, 0)
			end
		...
		for i = 1, #split do
			...
			TBS_FontDrawer(v, 'SA2TTFONT', FixedDiv((96-(#split[i] > 8 and #split[i]*2 or 0)+easespin-spliteaseout)*FRACUNIT, FRACUNIT+FRACUNIT/2), FixedDiv((88+(i-1)*20)*FRACUNIT, FRACUNIT+FRACUNIT/2), FRACUNIT+FRACUNIT/2, split[i], hudinfo[HUD_LIVES].f|V_PERPLAYER, v.getColormap(TC_DEFAULT, SKINCOLOR_SLATE), 0, -1, 0)
		end

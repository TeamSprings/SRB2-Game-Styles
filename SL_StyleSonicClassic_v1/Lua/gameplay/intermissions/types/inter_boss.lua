...
function inter.counterSetup(player)
	player.styles_tallyCounterTime = InterCalc.Y_GetTimeBonus(5 * max(player.realtime + (player.style_additionaltime or 0) - (player.styles_cutscenetime_prize or 0), 0))
	player.styles_tallyCounterGuard = InterCalc.Y_GetGuardBonus(player.timeshit)
	player.styles_tallyCounterTotal = 0
end

function inter.calculateALL(player)
	return (
        InterCalc.Y_GetTimeBonus(5 * max(player.realtime + (player.style_additionaltime or 0) - (player.styles_cutscenetime_prize or 0), 0)) +
        InterCalc.Y_GetGuardBonus(player.timeshit)
	)
end

function inter.counterWipe(player)
	player.styles_tallyCounterGuard = 0
...
function inter.counterThink(player)
	if player.styles_tallytimer and player.styles_tallytimer > 0 then
		...
		if player.styles_tallyCounterGuard and not player.styles_tallyCounterTime then
			player.styles_tallyCounterGuard = $-inter.calculationSpeed
			player.styles_tallyCounterTotal = $+inter.calculationSpeed

			if player.styles_tallyCounterGuard < 0 then
				player.styles_tallyCounterTotal = $+player.styles_tallyCounterGuard
				player.styles_tallyCounterGuard = 0
			end
...
function inter.draw(v, player, t, e)
	...
	v.draw(row3x, 124, v.cachePatch(fonts.font..'TGUARD'), V_PERPLAYER, v.getColormap(TC_DEFAULT, 1, color_profile.rings))
	...
	write(v, fonts.font..'TNUM', (row3x+160)*FU, 124*FU, FU, player.styles_tallyCounterGuard, V_PERPLAYER, v.getColormap(TC_DEFAULT, 1, color_profile.numbers), "right", fonts.padding)

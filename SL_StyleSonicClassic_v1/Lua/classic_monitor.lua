...
freeslot("SPR_MONITORS_CLASSIC", "S_DUMMYMONITOR", "S_DUMMYGMONITOR")
...
local monitor_cv = CV_RegisterVar{
	...
	func = function(var)
		local sets = {0, 3, 10, 6}
		states[S_DUMMYMONITOR].frame = sets[var.value]
...
local function P_SpawnItemBox(a)
	if a.info.flags & MF_GRENADEBOUNCE then
		a.state = S_DUMMYMONITOR --S_DUMMYGMONITOR
	else
		...
		if a.health > 0 then
			...
			if a.item and a.item.valid then
				...
				if (leveltime % 3) then
					...
					a.frame = states[a.state].frame
				else
					a.item.flags2 = $|MF2_DONTDRAW
					a.frame = states[a.state].frame+1
...
local function P_MonitorDeath(a, d, s)
	...
	if boxicon and boxicon.valid and a.flags & MF_NOGRAVITY then
		...
		smuk.fuse = 32
		smuk.scale = a.scale
		a.frame = states[a.state].frame + 2
		P_RemoveMobj(a.item)
	end

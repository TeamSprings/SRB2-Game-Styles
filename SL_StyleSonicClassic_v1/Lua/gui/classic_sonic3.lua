...
return{
		...
		if t and t <= 3*TICRATE then
			...
			if not mapheaderinfo[gamemap].s3bonusmode then
				...
				elseif mapheaderinfo[gamemap].subttl ~= "" then
					drawf(v, 'S3KTT', 302*FRACUNIT+tryx-offset*3, 104*FRACUNIT, FRACUNIT, string.upper(""..mapheaderinfo[gamemap].subttl), 0, v.getColormap(TC_DEFAULT, 1), "right")
				end
				if act ~= 0 then
			...
			drawf(v, 'S3KTT', 288*FRACUNIT+tryx-offset*3, 72*FRACUNIT, FRACUNIT, lvlt, 0, v.getColormap(TC_DEFAULT, 1), "right")
		end
	end,
	...
	lives = function(v, p, t, e)
		...
		drawf(v, 'HUS3NAM', (hudinfo[HUD_LIVES].x+17)*FRACUNIT, (hudinfo[HUD_LIVES].y+1)*FRACUNIT, FRACUNIT, string.upper(''..skins[p.mo.skin].hudname), hudinfo[HUD_LIVES].f|V_HUDTRANS|V_PERPLAYER, v.getColormap(TC_DEFAULT, 1), 0, 1)
		drawf(v, 'LIF3NUM', (hudinfo[HUD_LIVES].x+17+lifenamelenght)*FRACUNIT, (hudinfo[HUD_LIVES].y+9)*FRACUNIT, FRACUNIT, (p.lives == 127 and string.char(30) or p.lives), hudinfo[HUD_LIVES].f|V_HUDTRANS|V_PERPLAYER, v.getColormap(TC_DEFAULT, 1), "right", 1)
	end,
}